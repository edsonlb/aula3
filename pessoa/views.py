...
def usa_api_consulta(request):
    from django.shortcuts import HttpResponse
    import requests
    from requests.auth import HTTPDigestAuth

    r = requests.get('https://apiedson.herokuapp.com/api/pessoa/?nome__icontains=a&ordering=nome', auth=HTTPDigestAuth('admin', 'admin'))
    ...
    for tipo in pessoas:
        print tipo['nome'] + ' = ' + str(tipo['idade'])

    return HttpResponse('SELECT - OK')

def usa_api_incluir(request):
    from django.shortcuts import HttpResponse
    import requests
    from requests.auth import HTTPDigestAuth

    valores = {'nome':'Eita 2!', 'idade': '22', 'ano': '1'}

    r = requests.post('https://apiedson.herokuapp.com/api/pessoa/', valores, auth=HTTPDigestAuth('admin', 'admin'))

    print r.json()

    return HttpResponse('INSERT - OK')

def usa_api_excluir(request):
    from django.shortcuts import HttpResponse
    import requests
    from requests.auth import HTTPDigestAuth

    r = requests.delete('https://apiedson.herokuapp.com/api/pessoa/9', auth=HTTPDigestAuth('admin', 'admin'))

    print r

    return HttpResponse('DELETE - OK')

def usa_api_atualizar(request):
    from django.shortcuts import HttpResponse
    import requests
    from requests.auth import HTTPDigestAuth

    valores = {'id': '10','nome':'Opa', 'idade': '1', 'ano': '1'}

    r = requests.post('https://apiedson.herokuapp.com/api_manual/', valores, auth=HTTPDigestAuth('admin', 'admin'))
